 ...
 * @author phanh
 */
@WebServlet(name = "CurriculumDetailController", urlPatterns = {"/curriculumDetails"})
public class CurriculumDetailController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Subject_Mapping_PLO [][] map = dao.getMappingSubjectToPLO(cur);
            request.setAttribute("cur", cur);
            request.setAttribute("map", map);
            request.setAttribute("length", map.length);

        } catch (NumberFormatException e) {
            System.out.println("common.curriculum -> curriculumDetail -> doGet(): " + e);
        }

        request.getRequestDispatcher("gui/common/curriculum/detail.jsp").forward(request, response);
    }
